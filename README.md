...
Сборка контейнера:
```
 sudo docker build ./ -t <имя_контейнера> (например parkpass)
```

### Конфигурационный файл ```docker-compose``` ###
Используется ```PostgreSQL``` в качестве базы данных (контейнер ```parkpass-postgres```)
```
version: '3'
networks:
    main_net:
...
services:
    ...
    parkpass-web:
        ...
        image: parkpass:latest (Тут указать тег-собранного контейнера)
        networks:
            - main_net
        restart: always
        environment:
            ...
            - POSTGRES_PASSWORD=parkpass
        ports:
            - "8080:80" (Запускаем на порту 8080)
```

Запуск контейнеров:
```
sudo docker-compose up (Создает и запускает контейнеры)
```
...
Посмотреть запущенные контейнеры docker-compose:
```
sudo docker-compose ps
```

Или посмотреть все запущенные контейнеры на машине:
```
sudo docker ps (Там будут еще frontend-контейнеры и прочее)
```

### Последоватьльность обновления: ###
Простейший вариант:
1) Проверяем название образа из которого создается контейнер:
   ```
      parkpass-web:
        image: parkpass:latest
   ```
2) Собираем из обновленного репозитория образ:
   ```
      sudo docker build ./ -t parkpass (latest цепляется автоматом)
   ```
   В этот момент с текущего образа будет снят тег ```parkpass```
3) После сборки смотрим все контейнеры docker-compose и видим что-то такое:
    ```
    CONTAINER ID        IMAGE                 COMMAND                  CREATED             STATUS              PORTS                    NAMES
    3f30411e7774        9bfe15e20af1   "/bin/sh -c ./run_se…"   4 days ago          Up 4 days           0.0.0.0:8080->80/tcp     parkpass-web
    ```
4) Запоминаем ```9bfe15e20af1```, если хотим иметь возможность откатиться или присваиваем ему тег:
    ```
        sudo docker tag 9bfe15e20af1 parkspass:previous
    ```
5) Оcтанавливаем и удаляем контейнеры c down, запускам командой up

6) Если хотим запустить старый контейнер - меняем в docker-compose.yml файле parkpass-web.image свойство
