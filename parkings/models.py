...
from valet.utils.valet_notification_center import (
    VALET_NOTIFICATION_REQUEST_FOR_DELIVERY,
    ValetNotificationCenter,
    VALET_NOTIFICATION_REQUEST_CANCEL,
    VALET_NOTIFICATION_SET_RESPONSIBLE,
    VALET_NOTIFICATION_REQUEST_ACCEPT,
)
...
class Parking(models.Model):
    ...
    CURRENCY_LIST = (("RUB", "RUB"), ("KZT", "KZT"), ("USD", "USD"))
    ...
    parkpass_status = models.IntegerField(
        choices=PARKPASS_STATUSES, default=DISCONNECTED
    )
    free_places = models.IntegerField(default=0)
    max_places = models.IntegerField(default=0)
    max_permitted_time = models.IntegerField(
        default=3600, help_text="Max offline time in minutes"
    )
    ...
    owner = models.ForeignKey(
        "owners.Owner", null=True, blank=True, on_delete=models.CASCADE
    )
    company = models.ForeignKey(
        to="owners.Company", null=True, blank=True, on_delete=models.CASCADE
    )
    valet_email = models.CharField(max_length=200, null=True, blank=True)
    valet_telegram_secret_key = models.CharField(
        max_length=128, blank=True, null=True, default=uuid.uuid4
    )

    created_at = models.DateField(auto_now_add=True)
    software_updated_at = models.DateField(blank=True, null=True)
    approved = models.BooleanField(
        default=False, verbose_name="Is approved by administrator"
    )
    tariff = models.CharField(
        max_length=2000, default="{}", verbose_name="Tariff object JSON"
    )
    ...
    commission_client = models.BooleanField(
        default=False, verbose_name="Комиссию оплачивает клиент"
    )
    commission_client_value = models.IntegerField(
        default=0, verbose_name="Размер комиссии в процентах"
    )
    card_commission_client_value = models.IntegerField(
        default=0, verbose_name="Размер комиссии в процентах (карты)"
    )

    hide_parking_coordinates = models.BooleanField(
        default=False, verbose_name="Скрыть парковку с карты"
    )
    ...
    tz_name = models.CharField(
        choices=ALL_TIMEZONES, max_length=64, default=DEFAULT_PARKING_TIMEZONE
    )
    ...
    def set_valet_telegram_chat(self, chat_id):
        chat, _ = ParkingValetTelegramChat.objects.get_or_create(
            parking=self,
            chat_id=chat_id,
        )
...
class ParkingSerializer(serializers.ModelSerializer):
    class Meta:
        model = Parking
        fields = ["id", "name", "address", "city", "picture"]
...
class Wish(models.Model):
    ...
    parking = models.ForeignKey(
        to=Parking, null=True, blank=True, on_delete=models.CASCADE
    )
    ...
    def add_account(self, account):
        ...
        try:
            ...
            top_parking_wish.save()
            for wish in Wish.objects.filter(parking=self.parking).select_related(
                "user"
            ):
...
class ParkingSession(models.Model):
    ...
    STATE_COMPLETED_BY_CLIENT = (
        6  # (STATE_STARTED_BY_VENDOR + COMPLETED_BY_CLIENT_MASK)
    )
    STATE_COMPLETED_BY_CLIENT_FULLY = 7  # (STATE_STARTED + COMPLETED_BY_CLIENT_MASK)

    STATE_COMPLETED_BY_VENDOR = (
        10  # (STATE_STARTED_BY_VENDOR + COMPLETED_BY_VENDOR_MASK)
    )
    ...
    def add_vendor_start_mark(self):
        self.state += (
            ...
            if not (self.state & self.STATE_STARTED_BY_VENDOR)
            else self.state
        )

    def add_client_complete_mark(self):
        self.state += (
            self.COMPLETED_BY_CLIENT_MASK
            if not (self.state & self.COMPLETED_BY_CLIENT_MASK)
            else self.state
        )

    def add_vendor_complete_mark(self):
        self.state += (
            self.COMPLETED_BY_VENDOR_MASK
            if not (self.state & self.COMPLETED_BY_VENDOR_MASK)
            else self.state
        )
    ...
    def get_debt(self):
        ...
        if (
            self.parking
            and self.parking.commission_client
            and self.parking.commission_client_value
        ):
...
signals.post_save.connect(
    receiver=create_test_parking, sender=Vendor
)  # Test parking creation, when vendor
...
class ProblemParkingSessionNotifierSettings(models.Model):
    ...
    report_emails = models.TextField(
        validators=(comma_separated_emails,), null=True, blank=True
    )
...
class ParkingValetSession(models.Model):
    ...
    state = models.PositiveSmallIntegerField(
        choices=VALET_SESSION_STATE_CHOICES, default=VALET_SESSION_RECEIVED_THE_CAR
    )
    ...
    responsible_for_reception = models.ForeignKey(
        "owners.CompanyUser",
        on_delete=models.SET_NULL,
        null=True,
        related_name="responsible_relate_table",
    )
    responsible_for_delivery = models.ForeignKey(
        "owners.CompanyUser",
        on_delete=models.SET_NULL,
        null=True,
        related_name="responsible_for_delivery_relate_table",
        blank=True,
    )
    ...
    car_delivered_by = models.ForeignKey(
        "owners.CompanyUser",
        on_delete=models.SET_NULL,
        null=True,
        blank=True,
    ...
    parking_card_session = models.ForeignKey(
        "rps_vendor.RpsParkingCardSession", on_delete=models.SET_NULL, null=True
    )
    manual_close = models.BooleanField(default=False)
    ...
    def get_debt_from_remote(self):
        ...
        if self.parking_card and parking_card_model.exists():
            ...
            if response:
                ...
                self.parking_card_get_at = datetime.datetime.fromtimestamp(
                    response["entered_at"] / 1000.0, pytz.timezone("Europe/Moscow")
    ...
    def book(self, delivery_date):
        ...
        ParkingValetSessionRequest.objects.create(
            valet_session=self, car_delivery_time=delivery_date, company=self.company
        )
        self.save()
        ...
        current_session = ParkingValetSession.objects.get(
            id=self.id
        )  # хак для того чтобы не обрабатывать значение даты в ручную, sorry

        now = datetime.datetime.now(timezone.utc)
        now_plus_30 = now + datetime.timedelta(minutes=30)

        if current_session.car_delivery_time and (
            current_session.car_delivery_time <= now_plus_30
        ):  # проверка на время < 30min
            ValetNotificationCenter(
                type=VALET_NOTIFICATION_REQUEST_FOR_DELIVERY, session=current_session
            ).run()

            request = ParkingValetSessionRequest.objects.get(
                id=current_session.request.id
            )
    ...
    def cancel_request_if_status_changed(self, state):
        ...
        if int(state) == VALET_SESSION_THE_CAR_IS_PARKED and self.state in [
            ...
            if self.responsible_for_delivery_id:
                old_responsible_user = CompanyUser.objects.get(
                    id=self.responsible_for_delivery_id
                )
    ...
    def create_request_if_status_changed(self, state, add_time_by_backend):
        ...
        if self.state == VALET_SESSION_THE_CAR_IS_PARKED and int(state) in [
            VALET_SESSION_REQUESTING_A_CAR_DELIVERY,
            VALET_SESSION_THE_PERSON_IN_CHARGE,
        ]:
...
class ParkingValetSessionImages(models.Model):
    ...
    type = models.PositiveSmallIntegerField(
        choices=(
            (PHOTOS_AT_THE_RECEPTION, "Фото при приеме"),
            (PHOTOS_FROM_PARKING, "Фото с парковки"),
        ),
...
class ParkingValetSessionImagesSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        fields = ["id", "valet_session", "type", "created_at", "img"]
...
class ParkingValetSessionRequest(models.Model):
    ...
    status = models.PositiveSmallIntegerField(
        choices=VALET_REQUESTS_STATE_CHOICES, default=VALET_REQUEST_ACTIVE
    )
    created_at = models.DateTimeField(auto_now_add=True, editable=False)
    ...
    accepted_by = models.ForeignKey(
        "owners.CompanyUser", on_delete=models.SET_NULL, null=True
    )
...
class ParkingValetSessionSerializer(serializers.ModelSerializer):
    ...
    photos = ParkingValetSessionImagesSerializer(
        source="parkingvaletsessionimages_set", many=True
    )
