...
class RpsParking(models.Model):
    ...
    def get_parking_card_debt(
        self, parking_card, debt=None, duration=None, enter_ts=None
    ):
        if debt is None or duration is None or enter_ts is None:
            get_logger(
                f"get_parking_card_debt: debt={str(debt)} or duration={str(duration)} call make_http from rps"
            )
            debt, enter_ts, duration = self._make_http_for_parking_card_debt(
                parking_card.card_id
            )
        get_logger(
            "Returns: debt=%s, duration=%s"
            % (
                debt,
                duration,
            )
    ...
    def get_parking_card_debt_for_developers(self, parking_card, developer_id):
        debt, enter_ts, duration = self._make_http_for_parking_card_debt(
            parking_card.card_id, developer_id
        )
        get_logger(
            "Returns: debt=%s, duration=%s"
            % (
    ...
    def _make_http_for_parking_card_debt(self, parking_card, developer_id=None):
        ...
        get_logger().info(
            "SEND REQUEST TO RPS %s" % self.get_parking_card_debt_url(query_str)
        )
            ...
            try:
                ...
                if r.status_code == 200:
                    ...
                    if result.get("status") == "OK":
                        ...
                        entered_at_ts = int(
                            time.mktime(entered_at.timetuple()) * 1000
                            + entered_at.microsecond / 1000
                        )
                        return (
                            result["amount"],
                            entered_at_ts,
                            seconds_ago if seconds_ago > 0 else 0,
                        )
...
class RpsParkingCardSession(models.Model):
    ...
    account = models.ForeignKey(
        Account, null=True, default=None, on_delete=models.CASCADE
    )
    ...
    def __str__(self):
        return "Parking RPS session %s %s" % (self.parking_card, self.parking_id)
    ...
    def notify_authorize(self, order, developer_id=None, rps_parking=None):
        ...
        prefix_query_str = "ticket_id=%s&amount=%s&FromPay=ParkPass" % (
            self.parking_card.card_id,
            int(order.sum),
        )
        ...
        rps_parking = (
            ...
            .filter(parking__id=self.parking_id)
            .first()
        )
        ...
        if order.payload:
            ...
            data = {
                "regularCustomerId": order.payload["card_id"],
                "amount": int(order.sum),
            }
            send_request_with_retries(
                url, "POST", retries=5, json=data, headers=headers
            )
            return True
        else:
            try:
                ...
                result = self._make_http_ok_status(
                    rps_parking.request_payment_authorize_url,
                    payload,
                    developer_id,
                    rps_parking,
                    headers=headers,
    ...
    def _make_http_ok_status(
        self, url, payload, developer_id=None, rps_parking=None, headers=None
    ):
                ...
                if r.status_code == 200:
                    ...
                    if result["status"] == "OK":
                        if result["leave_at"] is None or result["leave_at"] == "":
                            ...
                            return {
                                "success": True,
                                "leave_at": parse(result["leave_at"]).replace(
                                    tzinfo=None
                                ),
                            }
                else:
    ...
    def get_debt(self):
        ...
        if (
            parking
            and parking.commission_client
            and parking.card_commission_client_value
        ):
...
class RpsSubscription(models.Model):
    ...
    push_notified_about_expired = models.BooleanField(
        default=False, null=True, blank=True
    )
    push_notified_about_soon_expired = models.BooleanField(
        default=False, null=True, blank=True
    )
        ...
        if timezone.now() >= self.expired_at and self.active:
            ...
            if self.prolongation:
                new_subscription = RpsSubscription.objects.create(
                    ...
                    prolongation=True,
                    idts=self.idts,
                    id_transition=self.id_transition,
                )
                new_subscription.create_order_and_pay()

    def create_order_and_pay(self):
        order = Order.objects.create(
            sum=Decimal(self.sum), subscription=self, acquiring=self.parking.acquiring
        )
...
class Developer(models.Model):
    ...
    api_key = models.CharField(
        max_length=256, default=secrets.token_hex(24), unique=True
    )
    developer_id = models.CharField(
        max_length=128, default=str(uuid.uuid1()), unique=True
    )
...
DEVELOPER_LOG_TYPES = [
    (DEVELOPER_LOG_GET_DEBT, "Получение задолженности"),
...
DEVELOPER_STATUS_TYPES = [
    (DEVELOPER_STATUS_ERROR, "Error"),
    (DEVELOPER_STATUS_SUCCESS, "Success"),
]


class DevelopersLog(models.Model):
    parking = models.ForeignKey(
        to=RpsParking, on_delete=models.CASCADE, blank=True, null=True
    )
