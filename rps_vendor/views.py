...
from base.utils import (
    get_logger,
    clear_phone,
    datetime_from_unix_timestamp_tz,
    elastic_log,
)
...
from parkings.views import (
    CreateParkingSessionView,
    UpdateParkingSessionView,
    CancelParkingSessionView,
...
from rps_vendor.models import (
    ...
    Developer,
    DevelopersLog,
    DEVELOPER_LOG_GET_DEBT,
    DEVELOPER_STATUS_SUCCESS,
    DEVELOPER_LOG_CONFIRM,
    CARD_SESSION_STATE_DICT,
)
from rps_vendor.tasks import rps_process_updated_sessions
from rps_vendor.validators import (
    RpsCreateParkingSessionValidator,
    RpsUpdateParkingSessionValidator,
    RpsCancelParkingSessionValidator,
    RpsCompleteParkingSessionValidator,
    RpsUpdateListParkingSessionValidator,
    ParkingCardRequestBodyValidator,
    ParkingCardSessionBodyValidator,
    CreateOrGetAccountBodyValidator,
    SubscriptionUpdateBodyValidator,
    DeveloperCardSessionBodyValidator,
    ...
    def post(self, request, *args, **kwargs):
        if request.method == "POST":
            request.data["session_id"] = (
                str(request.data["client_id"]) + "&" + str(request.data["started_at"])
            )
    ...
    def post(self, request):
        ...
        try:
            ...
            e = ValidationException(
                ...
                "Parking with id %s for vendor '%s' not found"
                % (parking_id, request.vendor.name),
            )
...
class GetParkingCardDebtMixin:
    ...
    def post(self, request, *args, **kwargs):
        ...
        parking_card, _ = ParkingCard.objects.get_or_create(card_id=card_id)
        try:
            rps_parking = RpsParking.objects.select_related("parking").get(
...
class GetDeveloperParkingCardDebtMixin:
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            response_dict = rps_parking.get_parking_card_debt_for_developers(
                parking_card, developer_id
            )
...
class AccountInitPayment(LoginRequiredAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            card_session = RpsParkingCardSession.objects.get(id=card_session)
...
class InitPayDebtMixin:
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            response_dict = dict(
                client_uuid=str(new_client_uuid), order_id=str(order.id)
            )
            if result:
                card_session.state = STATE_INITED
                card_session.save()
                ...
                if order.acquiring != "homebank":
                    qr_result = TinkoffAPI(with_terminal="pcard").sync_call(
                        TinkoffAPI.GET_QR, {"PaymentId": result["payment_id"]}
                    )
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            if payment:
                if order.acquiring != "homebank":
                    qr_result = TinkoffAPI(with_terminal="pcard").sync_call(
                        TinkoffAPI.GET_QR, {"PaymentId": payment["payment_id"]}
                    )
                    if qr_result:
                        if qr_result.get("Success", False):
...
class ConfirmPayDeveloperDebt(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        get_logger().info(
            "ConfirmPayDeveloperDebt card_session_id %s" % card_session_id
        )
        ...
        try:
            ...
            status = "error"
            if order.parking_card_session.notify_authorize(
                order, developer_id, rps_parking
            ):
                status = "success"
                developer_log.status = DEVELOPER_STATUS_SUCCESS
                developer_log.save()
...
class InitPaymentMixin:
    ...
    def post(self, request, *args, **kwargs):
        ...
        enter_ts = datetime_to_timestamp(
            dt.strptime(parking_enter_time, "%Y-%m-%d %H:%M:%S.%f")
        )
        ...
        try:
            ...
            parking_card_debt = rps_parking.get_parking_card_debt(
                parking_card, debt=amount, duration=duration, enter_ts=enter_ts
            )
...
class GetCardSessionStatusMixin:
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            orders = Order.objects.filter(parking_card_session=card_session).order_by(
                "-created_at"
            )
            ...
            response_dict = {
                ...
                "leave_at": (
                    datetime_to_timestamp(card_session.leave_at)
                    if card_session.leave_at
                    else None
                ),
                "error": None,
            }
...
class ResetDeveloperToken(APIView):
    def get(self, request, *args, **kwargs):
        if request.user.is_superuser:
            ...
            messages.add_message(request, messages.SUCCESS, "Api key изменен")
...
class MockingGetParkingCardDebt(SignedRequestAPIView):
    def post(self, request, *args, **kwargs):
        ...
        response_dict = dict(
            card_id=card_id, parking_id=parking_id, duration=200, debt=10
        )
        return JsonResponse(response_dict, status=200)
...
class SubscriptionCallbackView(SignedRequestAPIView):
    def post(self, request, *args, **kwargs):
        ...
        rps_subscription = (
            RpsSubscription.objects.filter(id=int(subscription_id))
        ...
        if not rps_subscription:
            get_logger().warning(
                "RpsSubscription with %s does not exists " % subscription_id
            )
            return JsonResponse({"status": "OK"}, status=200)

        order = Order.objects.filter(
            authorized=True, subscription=rps_subscription
        ).first()
        if not order:
        ...
        if status:
            ...
            try:
                parsed_datetime = datetime.datetime.strptime(
                    str_expired_at, ISO_KOZULIN_FORMAT
                )
                parsed_timestamp = datetime_to_timestamp(parsed_datetime)
                tz_datetime = rps_subscription.parking.get_utc_parking_datetime(
                    parsed_timestamp
                )
            ...
            RpsSubscription.objects.filter(
                parking=rps_subscription.parking, account=rps_subscription.account
...
class SubscriptionUpdateView(SignedRequestAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        subscription_qs = RpsSubscription.objects.filter(
            account=account, parking=parking, active=True
        )
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not account:
            ...
            subscription = RpsSubscription.objects.filter(
                account=account, parking_id=parking_id, active=True
            ).first()
            if subscription:
        ...
        if title and text and mailing:
            if parking_id:
                ...
                sessions = ParkingSession.objects.filter(
                    parking=parking, client_id__isnull=False
                )
                if parkings_sessions_date:
                    sessions = sessions.filter(
                        started_at__gte=mailing.parkings_sessions_date
                    )
