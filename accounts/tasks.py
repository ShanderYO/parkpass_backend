...
def generate_current_debt_order(parking_session_id):
    ...
    try:
        active_session = ParkingSession.objects.select_related("parking").get(
        ...
        if (
            active_session.state not in ParkingSession.ACTUAL_COMPLETED_STATES
        ...
        if new_order_sum < 0:
            ...
            if last_order.acquiring == "homebank":
                ...
                payment = TinkoffPayment.objects.get(
                    order=last_order, status=PAYMENT_STATUS_AUTHORIZED
                )
                request_data = payment.build_cancel_request_data(
                    int(last_order.sum * 100)
                )
...
def count_refund_orders_for_session(parking_session):
    ...
    for order in orders:
        get_logger().info(
            "order id for refund %s sum %s" % (order.id, order.refunded_sum)
        )
                ...
                try:
                    if session_order.acquiring == "homebank":
                        payment = HomeBankPayment.objects.filter(
                            order=session_order, status=PAYMENT_STATUS_AUTHORIZED
                        )[0]
...
def force_pay(parking_session_id):
    ...
    try:
        ...
        not_authorized_orders = Order.objects.filter(
            session=active_session, authorized=False
        )
        not_paid_orders = Order.objects.filter(
            session=active_session, authorized=True, paid=False
        )
                ...
                if order.acquiring == "homebank":
                    ...
                    if payments.exists():
                        ...
                        if order.acquiring == "homebank":
                            ...
                            for payment in payments:
                                get_logger().info(
                                    "payments not exists | force pay close 2 - %s %s"
                                    % (payment.status, payment.payment_id)
                                )
                                get_logger().info(
                                    payment.status == PAYMENT_STATUS_AUTHORIZED
                                ...
                                if payment.status == PAYMENT_STATUS_AUTHORIZED:
                                    get_logger().info(
                                        "payments not exists | force pay close 3"
                                    )
                                    order.confirm_payment_homebank(payment)
                                    break
                        else:
        ...
        if ordered_sum < session.get_debt():
            ...
            if (
                ...
                if current_account_debt >= session.parking.max_client_debt:
                    order = Order(
                        sum=session.parking.max_client_debt,
                        session=session,
                        acquiring=session.parking.acquiring,
                    )
                    order.save()
...
def _init_refund(parking_session):
    ...
    remaining_sum = (
        parking_session.target_refund_sum - parking_session.current_refund_sum
    )

    orders = Order.objects.filter(
        session=parking_session, authorized=True, canceled=True, refund_request=False
    )
    ...
    for order in orders:
        ...
        if order.acquiring == "homebank":
            ...
            payment = TinkoffPayment.objects.get(
                order=order, status=PAYMENT_STATUS_AUTHORIZED
            )
            request_data = payment.build_cancel_request_data(int(refund * 100))
            result = TinkoffAPI().sync_call(TinkoffAPI.CANCEL, request_data)
            get_logger().info(result)

            if result.get("Status") == "REFUNDED" or result.get("Status") == "REVERSED":
                ...
                order.save()
            elif result.get("Status") == "PARTIAL_REFUNDED":
                order.refund_request = True
                order.refunded_sum = (
                    float(result.get("OriginalAmount", 0)) / 100
                    - float(result.get("NewAmount", 0)) / 100
                )
